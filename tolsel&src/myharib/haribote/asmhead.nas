...
BOTPAK	EQU		0x00280000		; my_bootpack的加载地址
...
LEDS	EQU		0x0ff1
VMODE	EQU		0x0ff2			; 关于颜色数量的信息。是多少位颜色？
SCRNX	EQU		0x0ff4			; 分辨率X
SCRNY	EQU		0x0ff6			; 分辨率Y
VRAM	EQU		0x0ff8			; 图形缓冲区的起始地址

		ORG		0xc200			; 这个程序将被加载到哪里
...
; 获取画面模式信息
		...
		MOV		CX,VBEMODE
		MOV		AX,0x107                 ; 0x4f01即可使用高分辨率模式，0x107为低分辨率模式
		INT		0x10
		CMP		AX,0x004f
		JNE		scrn320
...
scrn320:
		MOV		AL,0x13			; VGA图形，320x200x8位颜色
		MOV		AH,0x00
		INT		0x10
		MOV		BYTE [VMODE],8	; 记录屏幕模式（C语言所引用）
		MOV		WORD [SCRNX],320
		MOV		WORD [SCRNY],200
...
memcpy:
		...
		RET
; 如果不忘记添加地址大小前缀，memcpy也可以用字符串指令来写。
